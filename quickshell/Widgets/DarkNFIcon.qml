...
Item {
    ...
    // Try to use system nerd fonts, prioritizing Fira fonts
    readonly property var nerdFontFamilies: [
        "FiraCode Nerd Font",
    ...
    readonly property string fontFamily: {
        ...
        // Prioritize Fira fonts since user has them installed
        for (let i = 0; i < nerdFontFamilies.length; i++) {
            if (availableFonts.includes(nerdFontFamilies[i])) {
                return nerdFontFamilies[i]
            }
        }
        // Fallback to monospace if no nerd font found
        return "monospace"
    }
