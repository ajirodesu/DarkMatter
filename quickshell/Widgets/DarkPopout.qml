    ...
    Item {
        ...
        property real calculatedX: {
            ...
            if (root.objectName === "appDrawerPopout") {
                ...
                // If opened from dock, use "follow-trigger" positioning to align with trigger
                if (root.triggerSection === "dock") {
                    positionSetting = "follow-trigger"
                }
            } else if (root.objectName === "applicationsPopout") {
                positionSetting = SettingsData.applicationsPosition
            }
            
            var baseX
            var isFromDock = root.triggerSection === "dock"

            console.log("=== DARKPOPOUT POSITIONING DEBUG ===")
            console.log("objectName:", root.objectName)
            console.log("triggerSection:", root.triggerSection)
            console.log("isFromDock:", isFromDock)
            console.log("positioning:", positioning)
            console.log("positionSetting:", positionSetting)
            console.log("triggerX:", triggerX, "triggerY:", triggerY)
            
            if (isFromDock && (root.objectName === "controlCenterPopout" || root.objectName === "appDrawerPopout")) {
                ...
                console.log("=== DARKPOPOUT POSITIONING (dock widget:", root.objectName, ") ===")
                console.log("triggerX:", triggerX, "triggerY:", triggerY, "triggerWidth:", triggerWidth, "positioning:", positioning)
                console.log("calculated baseX:", baseX)
            } else if (positionSetting === "follow-trigger" || positionSetting === "") {
                if (positioning === "center") {
                    baseX = triggerX + (triggerWidth / 2) - (popupWidth / 2)
                } else if (positioning === "left") {
                    baseX = triggerX
                ...
                console.log("Using positionSetting positioning, baseX:", baseX)
            }

            console.log("Final baseX before offset:", baseX)
            var xOffset = 0
            // Only apply offset when NOT using follow-trigger positioning
            // (follow-trigger already positions relative to trigger, so offset shouldn't apply)
            if (positionSetting !== "follow-trigger" && positionSetting !== "") {
                if (root.objectName === "appDrawerPopout") {
                    if (!positionSetting.includes("left") && !positionSetting.includes("right")) {
                        xOffset = SettingsData.startMenuXOffset * (screenWidth - popupWidth) / 2
                    }
                } else if (root.objectName === "controlCenterPopout") {
                    // Only apply offset if not from dock
                    if (!isFromDock && !positionSetting.includes("left") && !positionSetting.includes("right")) {
            ...
            var finalX = Math.max(minX, Math.min(maxX, baseX + xOffset))
            console.log("Final calculated X position:", finalX, "(baseX:", baseX, "xOffset:", xOffset, "minX:", minX, "maxX:", maxX, ")")
            return finalX
        }
        property real calculatedY: {
            ...
            if (root.objectName === "appDrawerPopout") {
                positionSetting = SettingsData.appDrawerPosition
                if ((positionSetting === "follow-trigger" || positionSetting === "") && root.triggerSection === "dock") {
                    positionSetting = "bottom-left"
                }
            } else if (root.objectName === "controlCenterPopout") {
                positionSetting = SettingsData.controlCenterPosition
            } else if (root.objectName === "applicationsPopout") {
            ...
            if (positionSetting === "follow-trigger" || positionSetting === "") {
                ...
                if (root.objectName === "appDrawerPopout" || root.objectName === "controlCenterPopout" || root.objectName === "applicationsPopout" || root.objectName === "volumeMixerPopout" || root.objectName === "dockVolumeMixerPopout") {
                    if (isFromDock) {
                        baseY = triggerY - popupHeight - Theme.spacingS
                        yOffset = 0
                    } else if (barPosition === "bottom" && !isBarVertical) {
                        baseY = triggerY - popupHeight - Theme.spacingS
                ...
                if (isFromDock && root.objectName === "controlCenterPopout") {
                    baseY = triggerY - popupHeight - Theme.spacingS
                    yOffset = 0
                } else if (positionSetting.includes("top")) {
                    ...
                    if (typeof SettingsData !== "undefined" && SettingsData.dockExclusiveZone > 0) {
                        var dockThickness = SettingsData.dockExclusiveZone + (SettingsData.dockBottomGap || 0) + ((SettingsData.dockTopPadding || 0) * 2)
                        baseY = actualScreenHeight - popupHeight - dockThickness - Theme.spacingL
                    }
            ...
            if (typeof SettingsData !== "undefined" && SettingsData.dockExclusiveZone > 0) {
                var positionSettingForDock = ""
                if (root.objectName === "appDrawerPopout") {
                    positionSettingForDock = SettingsData.appDrawerPosition
                } else if (root.objectName === "controlCenterPopout") {
                    positionSettingForDock = SettingsData.controlCenterPosition
                }
                
                var shouldAccountForDock = root.objectName === "dockVolumeMixerPopout" || 
                                          root.objectName === "dockApplicationsPopout" || 
                                          root.objectName === "controlCenterPopout" ||
                                          root.objectName === "appDrawerPopout" ||
                                          (positionSettingForDock && positionSettingForDock.includes("bottom"))
                
                if (shouldAccountForDock) {
                    var dockThickness = SettingsData.dockExclusiveZone + (SettingsData.dockBottomGap || 0) + ((SettingsData.dockTopPadding || 0) * 2)
                    maxY = actualScreenHeight - popupHeight - dockThickness - Theme.spacingM
                }
            }
            ...
            if (typeof SettingsData !== "undefined" && SettingsData.topBarPosition === "bottom" && !SettingsData.topBarFloat && SettingsData.topBarVisible) {
                ...
                if ((root.objectName === "appDrawerPopout" || root.objectName === "controlCenterPopout" || root.objectName === "volumeMixerPopout") && finalY > maxY) {
                    finalY = maxY
                }
            }
