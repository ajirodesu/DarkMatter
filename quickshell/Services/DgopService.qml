...
Singleton {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    ...
                    if (freq > 0) {
                        root.cpuFrequency = freq / 1000000.0
                    }
                } catch (e) {
                }
            }
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    ...
                    if (typeof LoggingService !== 'undefined') {
                        LoggingService.warn("DgopService", "Failed to parse fallback CPU info", { error: e.message })
                    }
                }
            }
        }
    ...
    Component.onDestruction: {
        ...
        if (fallbackUpdateTimer.running) {
            fallbackUpdateTimer.stop()
        }
        if (dgopProcess.running) {
            dgopProcess.running = false
        }
