...
Singleton {
    ...
    Component.onDestruction: {
        if (nmStateMonitor.running) {
            nmStateMonitor.running = false
        }
        if (refreshTimer.running) {
            refreshTimer.stop()
        }
        if (wifiScanTimer.running) {
            wifiScanTimer.stop()
        }
        const processes = [
            nmStateMonitor, nmDeviceMonitor, wifiScanner, wifiConnector, wifiDisconnector,
            networkForgetter, macAddressSetter
        ]
