...
Singleton {
        ...
        onExited: exitCode => {
            if (exitCode === 0) {
                ...
                if (typeof LoggingService !== 'undefined') {
                    LoggingService.warn("KeyboardService", "Immediate layout process exited with error", { exitCode: exitCode })
                }
            }
        }
    }
    ...
    Component.onDestruction: {
        ...
        if (layoutListProcess.running) {
            layoutListProcess.running = false
        }
        if (variantListProcess.running) {
            variantListProcess.running = false
        }
        if (localeListProcess.running) {
            localeListProcess.running = false
        }
        if (setLayoutProcess.running) {
            setLayoutProcess.running = false
        }
        if (setLocaleProcess.running) {
            setLocaleProcess.running = false
        }
        if (applyLayoutsProcess.running) {
            applyLayoutsProcess.running = false
        }
        if (immediateLayoutProcess.running) {
            immediateLayoutProcess.running = false
        }
    }
}
