...
Singleton {
    ...
    Component.onDestruction: {
        if (codecQueryProcess.running) {
            codecQueryProcess.running = false
        }
        if (codecFullQueryProcess.running) {
            codecFullQueryProcess.running = false
        }
        if (codecSwitchProcess.running) {
            codecSwitchProcess.running = false
        }
    }
}
