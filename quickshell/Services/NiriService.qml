...
Singleton {
    ...
    Process {
            ...
            onStreamFinished: {
                try {
                    ...
                    if (typeof LoggingService !== 'undefined') {
                        LoggingService.error("NiriService", "Failed to parse outputs data", { error: e.message })
                    }
                }
            }
        }
        ...
        onExited: exitCode => {
            if (exitCode !== 0) {
                if (typeof LoggingService !== 'undefined') {
                    LoggingService.warn("NiriService", "Outputs process exited with error", { exitCode: exitCode })
                }
            }
        }
    }
    ...
    function switchToWorkspace(workspaceIndex) {
        if (typeof workspaceIndex !== 'number' || isNaN(workspaceIndex) || workspaceIndex < 0) {
            if (typeof LoggingService !== 'undefined') {
                LoggingService.warn("NiriService", "Invalid workspace index", { workspaceIndex: workspaceIndex })
            }
            return false
        }
