...
Singleton {
    ...
    Process {
        ...
        onExited: exitCode => {
            if (exitCode !== 0) {
                if (typeof LoggingService !== 'undefined') {
                    LoggingService.warn("AwwwService", "Wallpaper set process exited with error", { exitCode: exitCode })
                }
            }
        }
        stderr: StdioCollector {
            onStreamFinished: {
                if (text && text.trim()) {
                    if (typeof LoggingService !== 'undefined') {
                        LoggingService.warn("AwwwService", "Wallpaper set process stderr", { stderr: text.trim() })
                    }
                }
            }
        }
    ...
    Process {
        ...
        onExited: exitCode => {
            if (exitCode !== 0) {
                if (typeof LoggingService !== 'undefined') {
                    LoggingService.warn("AwwwService", "Wallpaper clear process exited with error", { exitCode: exitCode })
                }
            }
        }
    }
    ...
    Component.onDestruction: {
        if (awwwCheckProcess.running) {
            awwwCheckProcess.running = false
        }
        if (daemonCheckProcess.running) {
            daemonCheckProcess.running = false
        }
        if (daemonStartProcess.running) {
            daemonStartProcess.running = false
        }
        if (wallpaperSetProcess.running) {
            wallpaperSetProcess.running = false
        }
        if (wallpaperClearProcess.running) {
            wallpaperClearProcess.running = false
        }
    }
}
