...
Singleton {
            ...
            onStreamFinished: {
                ...
                for (const line of lines) {
                    ...
                    if (parts.length >= 3 && (parts[2] === "vpn" || parts[2] === "wireguard")) {
                        ...
                        const autoconnect = parts.length >= 5 ? (parts[4] === "yes") : false
                        out.push({ name: parts[0], uuid: parts[1], type: parts[2], serviceType: svc, autoconnect: autoconnect })
                    }
                }
                root.profiles = out
            }
    ...
    function connect(uuidOrName) {
        ...
        if (uuidOrName.length > 256) {
            root.errorMessage = "Connection identifier too long"
            return
        }
        ...
        if (root.singleActive) {
            ...
            const escaped = ShellUtils.escapeShellArg(uuidOrName)
