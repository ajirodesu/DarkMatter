...
Item {
    ...
    function loadArtwork(url) {
        ...
        if (url.startsWith("http://") || url.startsWith("https://")) {
            const filename = "/tmp/.darkshell/trackart_" + Date.now() + ".jpg"
            activeTrackArtFile = filename

            cleanupProcess.command = ["sh", "-c", "mkdir -p /tmp/.darkshell && find /tmp/.darkshell -name 'trackart_*' ! -name '" + filename.split('/').pop() + "' -delete"]
            ...
            if (url.startsWith("file://")) {
                if (url.includes(".org.chromium.") || url.includes("/tmp/") || url.includes("/var/tmp/")) {
                    return
                }
            }
        ...
        onStatusChanged: {
            if (status === Image.Ready) {
                ...
                if (!source.toString().includes(".org.chromium.") &&
                    ...
                    console.warn("MediaPlayerTab: Failed to load artwork:", source)
                }
            }
        }
    }
