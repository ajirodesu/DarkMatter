...
DarkOSD {
    ...
    function executeCommand(command) {
        ...
        if (trimmedCommand.startsWith("cd ")) {
            ...
            const safeCommand = ShellUtils.buildSafeCdCommand(targetDir)
            if (!safeCommand) {
                addOutput("Error: Invalid directory path", true)
                return
            }
            commandRunning = true
            cdProcess.command = ["sh", "-c", safeCommand]
            cdProcess.running = true
            return
        }
        
        if (!ShellUtils.isValidCommand(trimmedCommand)) {
            addOutput("Error: Invalid command", true)
            return
        }
        
        commandRunning = true
        const safeCommand = ShellUtils.buildSafeCommand(trimmedCommand)
        if (!safeCommand) {
            addOutput("Error: Failed to build safe command", true)
            commandRunning = false
            return
        }
        commandProcess.command = ["sh", "-c", safeCommand]
