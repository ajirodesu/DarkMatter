                                ...
                                Rectangle {
                                    ...
                                    RowLayout {
                                        ...
                                        DarkActionButton {
                                            buttonSize: 28
                                            circular: true
                                            iconName: (modelData.autoconnect || false) ? "link" : "link_off"
                                            iconSize: 16
                                            iconColor: (modelData.autoconnect || false) ? Theme.primary : Theme.surfaceVariantText
                                            Layout.alignment: Qt.AlignVCenter
                                            onClicked: {
                                                if (VpnService) {
                                                    const currentState = modelData.autoconnect || false
                                                    const newState = !currentState
                                                    VpnService.setAutoconnect(modelData.uuid, newState)
                                                    ToastService.showInfo(newState ? "Auto-connect enabled for " + modelData.name : "Auto-connect disabled for " + modelData.name)
                                                }
                                            }
                                        }
