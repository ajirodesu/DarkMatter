...
build_once() {
  ...
  SETTINGS_FILE="$HOME/.config/DarkMaterialShell/settings.json"
  if [ -f "$SETTINGS_FILE" ]; then
    HYPRLAND_ENABLED=$(jq -r '.hyprlandThemingEnabled // true' "$SETTINGS_FILE" 2>/dev/null || echo "true")
  fi
  ...
  sed -i "s|input_path = '/home/matt/.config/quickshell/matugen/templates/|input_path = '$SHELL_DIR/matugen/templates/|g" "$TMP_CFG"
  ...
  if command -v hyprctl >/dev/null 2>&1 && [ "$HYPRLAND_ENABLED" = "true" ]; then
    PRIMARY_HEX=$(echo "$JSON" | jq -r '.colors.primary.dark' 2>/dev/null)
    SURFACE_HEX=$(echo "$JSON" | jq -r '.colors.surface.dark' 2>/dev/null)

    BORDER_SIZE=4
    SETTINGS_FILE="$HOME/.config/DarkMaterialShell/settings.json"
    if [ -f "$SETTINGS_FILE" ]; then
      BORDER_SIZE=$(jq -r '.hyprlandBorderSize // 4' "$SETTINGS_FILE" 2>/dev/null || echo "4")
    fi

    if [ -n "$PRIMARY_HEX" ] && [ -n "$SURFACE_HEX" ] && [ "$PRIMARY_HEX" != "null" ] && [ "$SURFACE_HEX" != "null" ]; then
      PRIMARY_RGB=$(echo "$PRIMARY_HEX" | sed 's/#//')
      SURFACE_RGB=$(echo "$SURFACE_HEX" | sed 's/#//')

      sed -i "s/col\.active_border = rgb([0-9a-fA-F]\{6\})/col.active_border = rgb($PRIMARY_RGB)/" ~/.config/hypr/hyprland/colors.conf
      sed -i "s/col\.inactive_border = rgb([0-9a-fA-F]\{6\})/col.inactive_border = rgb($SURFACE_RGB)/" ~/.config/hypr/hyprland/colors.conf
      sed -i "s/border_size = [0-9]\+/border_size = $BORDER_SIZE/" ~/.config/hypr/hyprland/colors.conf
